...
Original file is located at
    https://colab.research.google.com/drive/1w2EToZ06fp7KYNZc-Zwsq9h6WoZsr97z
...
class UnigramModel(LanguageModel):
    def __init__(self, trainCorpus):
        ...
        for sen in trainCorpus:
          for word in sen:
            if word != START:
              self.vab[word] = self.vab.get(word,0) + 1
              self.total += 1
            else:
              self.count += 1        
    ...
    def generateSentence(self):
        ...
        while curr != END:
          temp = random.random()
          for word in self.vab.keys():
            temp -= self.vab[word] / self.total
            if temp <= 0:
                break
          curr = word
          result.append(curr)
...
class SmoothedUnigramModel(LanguageModel):
    ...
    def generateSentence(self):
        ...
        while curr != END:
          temp = random.random()
          for word in self.vab.keys():
            temp -= (self.vab[word]+1)/(self.total+len(self.vab.keys()))
            if temp <= 0:
              break
          curr = word
          result.append(curr)
...
class BigramModel(LanguageModel):
    def __init__(self, trainCorpus):
        ...
        for sen in trainCorpus:
          for word in sen:
            if word != START:
              self.total +=1
              self.vocab_1[word] = self.vocab_1.get(word, 0) + 1
              temp = word+" "+curr
              self.vocab_2[temp] = self.vocab_2.get(temp, 0) + 1
            else:
              self.count+=1
            curr = word
    ...
    def generateSentence(self):
        ...
        while curr != END:
          temp = random.random()
          for word in self.vocab_1.keys():
            if curr == START:
              num = self.count
            else:
              num = self.vocab_1[curr]
            temp -= self.vocab_2.get(word+" "+curr, 0) / num
            if temp <= 0:
              break
          curr = word
          result.append(curr)
    ...
    def getSentenceLogProbability(self, sentence):
        ...
        for word in sentence:
          if word == START:
            curr = word
            continue
          if self.vocab_2.get(word+" "+curr, 0) == 0:
            return -float('inf')
          if curr == START:
            num = self.count
          else:
        ...
        for sen in testCorpus:
          for word in sen:
            if word == START:
              prev = word
              continue
            if self.vocab_2.get(word + " " + prev, 0) == 0:
              return float("Inf")
            if prev == START:
              num = self.count
            else:
              num = self.vocab_1[prev]
...
def buildFST():
    ...
    f.addSetTransition('q1', CONS.union(U), 'q_dE')
    f.addSetTransition('q1', E, 'q_EE')
    f.addSetTransition('q1', O, 'q_OE')
    ...
    f.addSetTransition('q_CONS', VOWS-E, 'q_CONS+VOWS-E')
    ...
    f.addTransition('q_dE', 'e', '', 'q_ing')

    f.addSetTransition('q_EE', E, 'q_ing')

    f.addSetTransition('q_OE', E, 'q_ing')

    f.addTransition('q_IE', 'e', 'y', 'q_ing')

    f.addSetTransition('q_CONS+E', AZ-DGMPT-VOWS, 'q_ing')

    f.addSetTransition('q_CONS+VOWS-E', AZ-BNR-DGMPT-VOWS, 'q_ing')
    ...
    for i in BNR.union(DGMPT):
        f.addTransition('q_CONS+VOWS-E', i, i*2, 'q_ing')
